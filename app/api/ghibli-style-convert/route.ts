  ...
  try {
    ...
    if (availableCredits < creditsNeeded) {
      return NextResponse.json({ 
        ...
        message: `您需要 ${creditsNeeded} Credits 来转换 ${n} 张 ${quality} 质量的吉卜力风格图片，但您只有 ${availableCredits} Credits 可用。`
      }, { status: 400 })
    }
    ...
    // 创建基本提示词，根据风格强度调整
    let ghibliPrompt = "Convert this image to Studio Ghibli animation style";
    
    // 根据风格强度增强或减弱提示词
    if (styleStrength === 'subtle') {
      ghibliPrompt = "Convert this image to a subtle Studio Ghibli animation style while preserving most of the original features, use soft watercolor textures and gentle lighting similar to Ghibli films";
    } else if (styleStrength === 'strong') {
      ghibliPrompt = "Convert this image to a strong Studio Ghibli animation style with classic Hayao Miyazaki aesthetics, detailed backgrounds, warm color palette, and dreamy atmosphere like in Spirited Away, My Neighbor Totoro, and Princess Mononoke";
    } else {
      ghibliPrompt = "Transform this image into Studio Ghibli animation style with hand-drawn aesthetics, watercolor backgrounds, and the signature Ghibli character design with expressive eyes";
    }

    // 如果用户提供了自定义提示词，则与风格提示词结合
    if (prompt && prompt !== 'Convert this image to Studio Ghibli animation style') {
      ghibliPrompt = `${ghibliPrompt}. ${prompt}`;
    }

    // Create FormData for Azure API
    const azureFormData = new FormData()
    azureFormData.append('image', imageFile)
    azureFormData.append('prompt', ghibliPrompt)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Studio Ghibli style conversion failed, please try again later' },
        { status: 500 }
      )
    }
    ...
    // 生成成功后消耗credits
    const generationId = `ghibli_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    const { error: transactionError } = await supabase
      ...
      .insert({
        ...
        description: `吉卜力风格转换${n}张${quality}质量图片`,
    ...
    console.error('Studio Ghibli style conversion error:', error)
    return NextResponse.json(
