...
interface GhibliStyleConverterProps {
  onImagesConverted: (images: string[]) => void
}

export default function GhibliStyleConverter({ onImagesConverted }: GhibliStyleConverterProps) {
  ...
  const { user } = useUser()
  const [styleStrength, setStyleStrength] = useState('medium') // 风格强度选项
  ...
  const handleConvertImage = async () => {
    ...
    try {
      ...
      formData.append('prompt', 'Convert this image to Studio Ghibli animation style')
            ...
            images.map(async (imageUrl: string) => {
              try {
                await database.saveImageGeneration({
                  ...
                  generationType: 'ghibli-style-convert'
                })
              } catch (error) {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h2 className="text-xl font-semibold text-gray-900">Studio Ghibli Style Converter</h2>
          {user && (
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
            <input
              ...
              className="hidden"
              id="image-upload-ghibli"
            />
            <label 
              htmlFor="image-upload-ghibli" 
        ...
        <div className="bg-gray-50 rounded-md p-4 border border-gray-200">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Ghibli Style Strength</label>
              <select
                value={styleStrength}
              ...
              >
                ...
                <option value="medium">Medium (Balanced)</option>
                <option value="strong">Strong (Full Ghibli Style)</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
          <h4 className="text-sm font-medium text-blue-800 mb-2 flex items-center gap-2">
            ...
            About Studio Ghibli Style Conversion
          </h4>
          <p className="text-xs text-blue-700 mb-3">
            This tool uses AI to transform your photos into the magical Studio Ghibli animation style. 
            Upload portraits, landscapes, or any image to see them converted into 
            beautiful Ghibli-inspired artwork with the signature hand-painted watercolor aesthetic.
          </p>
          <div className="text-xs text-gray-600">
            <p className="mb-1 font-medium">Best results with:</p>
            <ul className="list-disc list-inside space-y-1 text-gray-600">
              <li>Images with natural elements (forests, water, skies)</li>
              <li>Portraits with simple, clean backgrounds</li>
              <li>Photos with soft lighting and balanced colors</li>
              <li>Scenery photos with depth and interesting composition</li>
            </ul>
          </div>
        </div>
      </div>
